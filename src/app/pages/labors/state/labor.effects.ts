...
export class LaborEffects {
    ...
    @Effect({ dispatch: false })
    getTopListLabor$ = this.actions$
        .ofType('APP_GET_TOP_LIST_COPY')
        .do((action) => {
            this._spinner.show();
            this.LaborService.getTopList(action.payload).subscribe((result) => {
                this._spinner.hide();
                if (result.message == 'Action complete.' || result.statusCode == 200) {
                    let payload = result.data;
                    this.store.dispatch(new labor.AppGetTopListSuccess(payload));
                }
            }
                , (error) => {
                    this._spinner.hide();
    ...
    @Effect({ dispatch: false })
    getTopListLaborSuccess: Observable<Action> = this.actions$
        .ofType('APP_GET_TOP_LIST_COPY_SUCCESS')
        .do((action) => {
